<script setup>
const props = defineProps(['address_id','refresh'])
...
async function handleDelete() {
    ...
    try {
        await $fetch('/api/profile/addresses/delete',{
            method: 'POST',
            body: {address_id: props.address_id}
        })
        push.success('آدرس شما با موفقیت حذف شد')
        props.refresh()
    } catch (error) {
...
<template>
    ...
    <button @click="handleDelete" type="button" class=" btn-custom inline-flex  items-center gap-x-1  text-sm">
        حذف
