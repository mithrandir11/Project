    ...
    <nav class="py-2.5">
        <div class="flex flex-wrap justify-between items-center mx-auto ">
            ...
            <div class="flex items-center lg:order-2">
                ...
                <NuxtLink v-if="authUser && authUser.user"  :to="{name: 'profile'}" class="hidden lg:block  font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2  border">{{ authUser.user.name }}</NuxtLink>
        ...
        <div class="hidden lg:block bg-primary-900 bg-opacity-20  p-2 rounded-b-lg">
            <ul v-if="categories" class="flex justify-center   lg:gap-x-24 lg:mt-0">
                <li>
                    <NuxtLink   active-class="bg-primary-900 bg-opacity-20 rounded-lg px-2" class="text-primary-950"  >همه</NuxtLink>
                </li>
                <li v-for="category in categories.data" :key="category.id">
                    <!-- <NuxtLink :to="`/category/${category.id}/${category.slug}`" active-class="bg-primary-900 bg-opacity-20 rounded-lg px-2" class="text-primary-950"  >{{ category.title }}</NuxtLink> -->
